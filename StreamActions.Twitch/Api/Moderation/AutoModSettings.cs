...
public sealed record AutoModSettings
{
    ...
    /// </para>
    /// <para>
    /// Only <see cref="OverallLevel"/> or the individual settings may be used, but not both.
    /// </para>
    /// <para>
    /// An overall setting of, for example, 4, does not necessarily mean that individual settings are also set to 4, but rather to a related severity of recommended defaults.
    /// </para>
    /// <para>
    /// If you try to set a single individual setting, but do not set the others, then the others will be set to 0.
    /// </para>
    /// <para>
    ...
    public static async Task<ResponseData<AutoModSettings>?> UpdateAutoModSettings(TwitchSession session, string broadcasterId, string moderatorId, AutoModSettings settings)
    {
        ...
        bool hasIndividualSettings = settings.Aggression.HasValue ||
                                      settings.Bullying.HasValue ||
                                      settings.Disability.HasValue ||
                                      settings.Misogyny.HasValue ||
                                      settings.RaceEthnicityOrReligion.HasValue ||
                                      settings.SexBasedTerms.HasValue ||
                                      settings.SexualitySexOrGender.HasValue ||
                                      settings.Swearing.HasValue;

        if (settings.OverallLevel.HasValue && hasIndividualSettings)
        {
            throw new ArgumentOutOfRangeException(nameof(settings), "You may set either OverallLevel or the individual settings, but not both.").Log(TwitchApi.GetLogger());
        }

        if (!settings.OverallLevel.HasValue && !hasIndividualSettings)
        {
            throw new ArgumentNullException(nameof(settings), "You must specify either OverallLevel or at least one individual setting.").Log(TwitchApi.GetLogger());
        }

        AutoModSettings clampedSettings = settings with
        {
            OverallLevel = settings.OverallLevel.HasValue ? Math.Clamp(settings.OverallLevel.Value, 0, 4) : null,
            Aggression = settings.Aggression.HasValue ? Math.Clamp(settings.Aggression.Value, 0, 4) : null,
            Bullying = settings.Bullying.HasValue ? Math.Clamp(settings.Bullying.Value, 0, 4) : null,
            Disability = settings.Disability.HasValue ? Math.Clamp(settings.Disability.Value, 0, 4) : null,
            Misogyny = settings.Misogyny.HasValue ? Math.Clamp(settings.Misogyny.Value, 0, 4) : null,
            RaceEthnicityOrReligion = settings.RaceEthnicityOrReligion.HasValue ? Math.Clamp(settings.RaceEthnicityOrReligion.Value, 0, 4) : null,
            SexBasedTerms = settings.SexBasedTerms.HasValue ? Math.Clamp(settings.SexBasedTerms.Value, 0, 4) : null,
            SexualitySexOrGender = settings.SexualitySexOrGender.HasValue ? Math.Clamp(settings.SexualitySexOrGender.Value, 0, 4) : null,
            Swearing = settings.Swearing.HasValue ? Math.Clamp(settings.Swearing.Value, 0, 4) : null
        };
        ...
        using JsonContent jsonContent = JsonContent.Create(clampedSettings, options: TwitchApi.SerializerOptions);
