...
public sealed record VIP
{
    ...
    /// <exception cref="TwitchScopeMissingException"><paramref name="session"/> does not have the scope <see cref="Scope.ChannelReadVips"/>, <see cref="Scope.ChannelManageVips"/>, or <see cref="Scope.ModeratorReadVips"/>.</exception>
    /// <remarks>
    /// <para>
    /// This ID must match the user ID in the access token.
    /// </para>
    /// <para>
    /// App token specific scope or moderator requirements: the app must have an authorization from <paramref name="broadcasterId"/> which includes the <see cref="Scope.ChannelReadVips"/>, <see cref="Scope.ChannelManageVips"/>, or <see cref="Scope.ModeratorReadVips"/> scope.
    /// </para>
    /// <para>
    /// Response Codes:
    /// <list type="table">
    ...
    public static async Task<ResponseData<VIP>?> GetVIPs(TwitchSession session, string broadcasterId, IEnumerable<string>? userId = null, int first = 20, string? after = null)
    {
        ...
        session.RequireUserOrAppToken(Scope.ChannelReadVips, Scope.ChannelManageVips, Scope.ModeratorReadVips);
    ...
    /// </summary>
    /// <param name="session">The <see cref="TwitchSession"/> to authorize the request.</param>
    /// <param name="broadcasterId">The ID of the broadcaster that's adding the user as a VIP.</param>
