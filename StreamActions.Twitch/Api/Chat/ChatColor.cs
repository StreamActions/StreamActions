...
public sealed record ChatColor
{
    ...
    public static async Task<ResponseData<ChatColor>?> GetUserChatColor(TwitchSession session, IEnumerable<string> userId)
    {
        ...
        if (!userId.TryGetNonEnumeratedCount(out int count))
        {
            count = userId.Count();
        }
        ...
        if (count > 100)
        {
            throw new ArgumentOutOfRangeException(nameof(userId), count, "must have a count <= 100").Log(TwitchApi.GetLogger());
        }
